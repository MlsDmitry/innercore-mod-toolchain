...
def search_ndk_path(home_dir):
    ...
    try:
        ...
        print_err("Please locate your ndk folder as described here -> https://github.com/zheka2304/innercore-mod-toolchain#installing-android-ndk")
...
def check_installed(arch):
    return os.path.isfile(make_config.get_path(os.path.normpath("toolchain/ndk/.installed-" + str(arch))))
...
def install(arch="arm", reinstall=False):
    if not reinstall and check_installed(arch):
        ...
        print(os.path.normpath(os.path.join(ndk_path, "build/tools/make_standalone_toolchain.py")))
        result = subprocess.call([
            "python",
            os.path.normpath(os.path.join(ndk_path, "build/tools/make_standalone_toolchain.py")),
            "--arch", str(arch),
            "--install-dir", make_config.get_path(os.path.normpath("toolchain/ndk/" + str(arch))),
            "--force"
        ])
        ...
        if result == 0:
            ...
            toolchain\\ndk\\.installed-
            """
            open(make_config.get_path(os.path.normpath("toolchain/ndk/.installed-" + str(arch))), 'tw').close()
            return True
        else:
